...
class I18nTest extends AbstractModifiedTest
{
    public function run()
    {
        $this->checkLanguages();
    ...
    {
        $result = (new Result())->setName('Modified Sprachen ISO-Codes');

        $languages = $this->Db()->query('SELECT COUNT(*) AS missing FROM languages WHERE code IS NULL || code = ""');

        if ($languages->fetchColumn() > 0) {
            $result->setError(
                (new Error())->setMessage('In Modified sind Sprachen angelegt die über keinen gültigen ISO-Code verfügen.')
    ...
    {
        $result = (new Result())->setName('Modified Kategorie-Übersetzungen Daten-Konsistenz');
        ...
        if ($categories->rowCount() > 0) {
            $result->setError(
                (new Error())->setMessage('Die Modified-Datenbank enthält inkonsistente Kategorie-Übersetzungen.')
                    ->setLevel(Error::LEVEL_CRITICAL)
    ...
    {
        $result = (new Result())->setName('Modified Produkt-Übersetzungen Daten-Konsistenz');
        ...
        if ($descriptions->rowCount() > 0) {
            $result->setError(
                (new Error())->setMessage('Die Modified-Datenbank enthält inkonsistente Produkt-Übersetzungen.')
                    ->setLevel(Error::LEVEL_CRITICAL)
